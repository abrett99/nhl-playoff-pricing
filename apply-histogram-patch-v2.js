#!/usr/bin/env node
// ============================================================================
// Patch v2: Uses exact anchor strings instead of brace counting
// ============================================================================
...
async function main() {
  ...
  // Backup
  await fs.writeFile(HTML_PATH + '.bak2', html);
  console.log('[patch-v2] Backup saved to', HTML_PATH + '.bak2');

  // Check if already patched
  if (html.includes('simulateSeriesBootstrap')) {
    throw new Error('Already contains simulateSeriesBootstrap - revert first! cp src/ui/index.html.bak src/ui/index.html');
  }

  // 1. Insert bootstrap BEFORE "function renderSeriesCard" (much safer than finding end of simulateSeries)
  const anchor1 = '\nfunction renderSeriesCard(series) {';
  const anchor1Idx = html.indexOf(anchor1);
  if (anchor1Idx === -1) throw new Error('Could not find renderSeriesCard anchor');
  console.log('[patch-v2] Inserting bootstrap code BEFORE renderSeriesCard at position', anchor1Idx);
  html = html.slice(0, anchor1Idx) + '\n' + BOOTSTRAP_CODE + html.slice(anchor1Idx);

  // 2. Replace renderSeriesCard - find it, then find the closing " + `;" pattern at end of template literal
  const rscIdx = html.indexOf('function renderSeriesCard(series) {');
  if (rscIdx === -1) throw new Error('Could not find renderSeriesCard');

  // The old function ends with:  `;\n}\n   then blank line or another function
  // Find first occurrence of "`;" after rscIdx followed by "\n}"
  let endIdx = -1;
  let searchFrom = rscIdx;
  while (searchFrom < html.length) {
    const tickIdx = html.indexOf('`;', searchFrom);
    if (tickIdx === -1) break;
    // Check if followed by \n}
    const after = html.slice(tickIdx + 2, tickIdx + 6);
    if (after.startsWith('\n}')) {
      endIdx = tickIdx + 2 + 2; // past `;\n}
      break;
    }
    searchFrom = tickIdx + 2;
  }
  if (endIdx === -1) throw new Error('Could not find end of renderSeriesCard template literal');
  console.log('[patch-v2] Replacing renderSeriesCard from', rscIdx, 'to', endIdx);
  ...
  console.log('[patch-v2] CSS added');

  // 4. Add tab click handler after goalie-override listener
  const goalieIdx = html.indexOf(".goalie-override')");
  if (goalieIdx !== -1) {
    const endMarker = '});';
    const endBlock = html.indexOf(endMarker, goalieIdx);
    if (endBlock !== -1) {
      const insertAt = endBlock + endMarker.length;
      html = html.slice(0, insertAt) + '\n' + TAB_HANDLER + html.slice(insertAt);
      console.log('[patch-v2] Tab handler added');
    } else {
      console.warn('[patch-v2] WARN: could not find end of goalie-override block');
    }
  } else {
    console.warn('[patch-v2] WARN: could not find goalie-override listener');
  }

  await fs.writeFile(HTML_PATH, html);
  console.log('[patch-v2] \u2713 Success! File written to', HTML_PATH);
  console.log('[patch-v2] Next: git add src/ui/index.html && git commit -m "Histogram + tabs v2" && git push');
}

main().catch(err => {
  console.error('[patch-v2] FAILED:', err.message);
  process.exit(1);
});
